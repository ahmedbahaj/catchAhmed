...
function setup() {
  ...
  mazeA[0][0] = mazeB[0][0] = 0; // Start
  mazeA[rows - 1][cols - 1] = mazeB[rows - 1][cols - 1] = 0; // Goal
...
function draw() {
  ...
  // Draw goal
  fill(255, 215, 0);
  rect((cols - 1) * tileSize, (rows - 1) * tileSize, tileSize, tileSize);
  ...
  for (let y = 0; y < rows; y++) {
    for (let x = 0; x < cols; x++) {
      if (currentMaze[y][x] === 1) {
        // Shadow
        fill(0, 0, 0, 60);
        rect(x * tileSize + 4, y * tileSize + 4, tileSize, tileSize);
        // Wall
        fill(100);
